...
router.post('/registration', (req, res) => {
    if (Object.keys(req.body).length === 0 || !req.body.username || !req.body.password) {
        return res.status(constants.BAD_REQUEST).send({ errorMessage: messages.BAD_REQUEST });
    }
    jsonfile.readFile(filePath, (err, obj) => {
        if (err) {
            return res.status(constants.SERVER_ERROR).send({ errorMessage: messages.SERVER_ERROR });
        }
        if (!validateUsername(req.body.username)) {
            return res.status(constants.BAD_REQUEST).send({ errorMessage: messages.ENTER_VALID_USERNAME });
        }
        if (existingUser(req.body.username, obj)) {
            return res.status(constants.CONFLICT).send({ errorMessage: messages.ENTER_VALID_USERNAME });
        }
        ...
        jsonfile.writeFile(filePath, obj, { spaces: 2, EOL: '\r\n' }, (writeErr) => {
            if (writeErr) {
                return res.status(constants.SERVER_ERROR).send({ errorMessage: messages.OK });
            }
            return res.status(constants.OK).send({ errorMessage: messages.OK });
        });
    });
});
...
router.post('/login', (req, res) => {
    ...
    jsonfile.readFile(filePath, (err, obj) => {
        ...
        if (!obj[ req.body.username ] || obj[ req.body.username ] !== hashedPassword) {
            return res.status(constants.UNAUTHORIZED_USER).send({ errorMessage: messages.UNAUTHORIZED_USER });
        }
